...
const analizeMobilNet_js_1 = __importDefault(require("../middleware/analizeMobilNet.js"));
...
router.post("/", imageFilter_1.default.single("zipFile"), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (algorithm == "CocoSSD") {
        ...
        let response = yield learning.recognition();
        res.send(response);
    }
    if (algorithm == "MobilNet") {
        const learning = new analizeMobilNet_js_1.default(files, searchWord, percentage, secondsToString, algorithm, pathImage);
        let response = yield learning.recognition();
        res.send(response);
    }
}));
