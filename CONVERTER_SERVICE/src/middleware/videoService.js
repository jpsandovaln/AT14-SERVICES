...
class VideoServices {
    ...
    paramObtainFrames() {
        var paramFrames = {
            frameScale:
                this.body.frameScale != undefined ? this.body.frameScale : "320",
            grayScale:
                this.body.grayScale != undefined ? this.body.grayScale : "false",
            timeBetweenFrames:
                this.body.timeBetweenFrames != undefined
                    ? this.body.timeBetweenFrames
                    : "1",
            framesFormat:
                this.body.framesFormat != undefined ? this.body.framesFormat : ".jpg",
        };
        return paramFrames;
    }
    ...
    changeVideoFormat() {
        const cmdVideoFormat = BuildCmdChangeVideoFormat.returnCommand(
            codecPath,
            this.filePath,
            this.paramVideoFormat(),
            videoPath,
            this.outputFormat
        );
        this.compiler.execute(cmdVideoFormat);
        const resultPathVideoFormat =
            videoPath + path.parse(this.nameFile).name + this.outputFormat;
    ...
    obtainAudio() {
        if (this.body.obtainAudio == "true") {
            ...
            const resultPathAudio =
                audioPath + path.parse(this.nameFile).name + ".mp3";
